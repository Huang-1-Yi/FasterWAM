...
export TASK_CONFIG="${TASK_CONFIG:-fold100_fasterwam_action_full_pilot}"
export RUN_ROOT=/media/sata4t/hy_fasterwam_runs/fold100
export RUN_ID="action_full_pilot_2gpu_$(date +%Y%m%d_%H%M%S)"
export OUTPUT_DIR="${RUN_ROOT}/${RUN_ID}"
mkdir -p "${OUTPUT_DIR}"
printf '%s\n' "${OUTPUT_DIR}" > "${RUN_ROOT}/latest_action_full_pilot_2gpu_run.txt"
...
echo "[action-full-pilot-2gpu] run_dir=${OUTPUT_DIR}"
