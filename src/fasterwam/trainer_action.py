...
class Wan22ActionTrainer:
    def __init__(self, model, train_dataset, val_dataset=None, *, cfg: DictConfig):
        self.model = model
        self.train_dataset = train_dataset
        self.val_dataset = val_dataset
        self.cfg = cfg
        self.training_scope = str(cfg.get("training_scope", "action_adapter")).strip().lower()
        if self.training_scope not in {"action_adapter", "action_full"}:
            raise ValueError(
                "Wan22ActionTrainer requires training_scope action_adapter or action_full, "
                f"got {self.training_scope!r}."
            )
        ...
        self.init_checkpoint = cfg.get("init_checkpoint")
        self.action_delta_checkpoint = cfg.get("action_delta_checkpoint")
        self.save_optimizer_state = bool(cfg.get("save_optimizer_state", False))
        ...
        # A weight-only checkpoint must be loaded before AdamW/DeepSpeed creates
        # FP32 master parameters.  Loading it after accelerator.prepare() would
        # leave the optimizer master weights based on the earlier initialization.
        self._load_initial_checkpoint_before_optimizer()

        # Freeze before optimizer/DeepSpeed initialization so no optimizer state is
        # allocated for the 5B video expert.  The action expert and the proprio
        # projection are the only trainable modules in this trainer.
        self._apply_training_scope(self.model, self.training_scope)
        trainable_params = [param for param in self.model.parameters() if param.requires_grad]
        if not trainable_params:
            raise RuntimeError("Action training selected, but no trainable parameters were found.")
        total_param_count = sum(param.numel() for param in self.model.parameters())
        trainable_param_count = sum(param.numel() for param in trainable_params)
        logger.info(
            "Action training scope=%s: trainable_params=%.6f B total_params=%.3f B (%.4f%% trainable)",
            self.training_scope,
        ...
        preview_limit = 40
        for name in trainable_names[:preview_limit]:
            logger.info("[trainable] %s", name)
        if len(trainable_names) > preview_limit:
            logger.info(
                "[trainable] ... %d additional tensors omitted from console; see parameter_manifest.json",
                len(trainable_names) - preview_limit,
            )
        video_trainable = sum(p.numel() for p in self.model.video_expert.parameters() if p.requires_grad)
        if video_trainable:
            raise RuntimeError(f"Video expert unexpectedly has {video_trainable} trainable parameters.")
        for frozen_group_name in ("vae", "text_encoder"):
            frozen_module = getattr(self.model, frozen_group_name, None)
            if frozen_module is None:
                continue
            unexpectedly_trainable = sum(
                p.numel() for p in frozen_module.parameters() if p.requires_grad
            )
            if unexpectedly_trainable:
                raise RuntimeError(
                    f"{frozen_group_name} unexpectedly has {unexpectedly_trainable} trainable parameters."
                )
        fusion_params = list(getattr(self.model.dit, "video_kv_fusion_logits", []))
        if not fusion_params or not all(p.requires_grad for p in fusion_params):
            raise RuntimeError("FasterWAM video_kv_fusion_logits are missing or frozen.")
        ...
        self.recent_global_losses = deque(maxlen=50)
        ...
        if self.accelerator.is_main_process:
            self._write_parameter_manifest(
    ...
    def _load_initial_checkpoint_before_optimizer(self):
        checkpoint = self.init_checkpoint
        if not checkpoint:
            raise ValueError("Action training requires `init_checkpoint`.")
        checkpoint_path = Path(str(checkpoint))
        if not checkpoint_path.is_file():
            raise FileNotFoundError(f"Initial checkpoint not found: {checkpoint}")
        logger.info("Loading initial FasterWAM checkpoint before optimizer initialization: %s", checkpoint)
        payload = torch.load(checkpoint_path, map_location="cpu")
        if "mot" not in payload:
            raise ValueError(f"Initial checkpoint has no `mot` state: {checkpoint}")
        incompat = self.model.mot.load_state_dict(payload["mot"], strict=False)
        if incompat.missing_keys or incompat.unexpected_keys:
            raise RuntimeError(
                "FasterWAM checkpoint/model mismatch: "
                f"missing={incompat.missing_keys[:20]} unexpected={incompat.unexpected_keys[:20]}"
            )
        if self.model.proprio_encoder is not None:
            if "proprio_encoder" not in payload:
                raise ValueError("Initial checkpoint has no `proprio_encoder` state.")
            self.model.proprio_encoder.load_state_dict(payload["proprio_encoder"], strict=True)
        logger.info("Initial checkpoint loaded with strict key compatibility before optimizer creation.")
        if self.action_delta_checkpoint:
            self._load_action_delta(self.action_delta_checkpoint)

    def _load_action_delta(self, checkpoint):
        checkpoint_path = Path(str(checkpoint))
        if not checkpoint_path.is_file():
            raise FileNotFoundError(f"Action delta checkpoint not found: {checkpoint}")
        payload = torch.load(checkpoint_path, map_location="cpu")
        if payload.get("format") != "fasterwam_action_delta_v1":
            raise ValueError(f"Unsupported action delta format: {payload.get('format')!r}")
        named_parameters = dict(self.model.named_parameters())
        delta = payload.get("trainable_state_dict", {})
        missing = sorted(set(delta) - set(named_parameters))
        if missing:
            raise RuntimeError(f"Action delta contains unknown parameter names: {missing[:20]}")
        with torch.no_grad():
            for name, value in delta.items():
                target = named_parameters[name]
                if tuple(value.shape) != tuple(target.shape):
                    raise RuntimeError(
                        f"Action delta shape mismatch for {name}: {tuple(value.shape)} vs {tuple(target.shape)}"
                    )
                target.copy_(value.to(device=target.device, dtype=target.dtype))
        logger.info("Loaded action delta %s with %d tensors.", checkpoint, len(delta))
    ...
    def _resume_training_state_if_requested(self):
        ...
        if resume_path.is_dir():
            ...
            return
        raise ValueError("`resume` must point to an Accelerate state directory, not a weight file.")

    def _set_training_scope(self):
        logger.info("Applying FasterWAM training scope: %s", self.training_scope)
        model = self.accelerator.unwrap_model(self.model)
        self._apply_training_scope(model, self.training_scope)

    @staticmethod
    def _module_parameter_summary(module):
        if module is None:
            return {
                "present": False,
                "total_parameters": 0,
        ...
        return {
            ...
            "trainable_tensors": sum(1 for param in parameters if param.requires_grad),
        }

    def _write_parameter_manifest(
        self,
        *,
        ...
        model = self.model
        fusion = getattr(model.dit, "video_kv_fusion_logits", None)
        manifest = {
            "training_scope": self.training_scope,
            "initialization": {
                "base_checkpoint": str(self.init_checkpoint),
                "action_delta_checkpoint": (
                    None if not self.action_delta_checkpoint else str(self.action_delta_checkpoint)
                ),
            },
            "totals": {
                "parameters": int(total_param_count),
                "trainable_parameters": int(trainable_param_count),
                "frozen_parameters": int(total_param_count - trainable_param_count),
                "trainable_percent": float(
                    100.0 * trainable_param_count / max(total_param_count, 1)
                ),
            },
            "logical_groups": {
                "video_expert": self._module_parameter_summary(
                    getattr(model, "video_expert", None)
                ),
                "action_expert": self._module_parameter_summary(
                    getattr(model, "action_expert", None)
                ),
                "proprio_encoder": self._module_parameter_summary(
                    getattr(model, "proprio_encoder", None)
                ),
                "vae": self._module_parameter_summary(getattr(model, "vae", None)),
                "text_encoder": self._module_parameter_summary(
                    getattr(model, "text_encoder", None)
                ),
                "video_kv_fusion_logits": self._module_parameter_summary(fusion),
            },
            "trainable_parameter_names": trainable_names,
            "frozen_guards": ["video_expert", "vae", "text_encoder"],
        }
        manifest_path = os.path.join(self.output_dir, "parameter_manifest.json")
        with open(manifest_path, "w", encoding="utf-8") as file:
            json.dump(manifest, file, ensure_ascii=False, indent=2)
        logger.info("Saved parameter freeze/train audit: %s", manifest_path)

    @staticmethod
    def _apply_training_scope(model, training_scope):
        if not hasattr(model, "action_expert"):
            raise AttributeError("Action trainer requires `model.action_expert`.")
        model.eval()
        ...
        fusion_params = getattr(model.dit, "video_kv_fusion_logits", None)
        if fusion_params is None:
            raise AttributeError("FasterWAM SparseMoT has no video_kv_fusion_logits.")
        fusion_params.requires_grad_(True)
    ...
    def _save_weights_checkpoint(self, step_tag: str):
        ...
        payload = {
            "format": "fasterwam_action_delta_v1",
            "training_scope": self.training_scope,
            "base_checkpoint": str(self.init_checkpoint),
        ...
        logger.info(
            "Saved lightweight action delta with %d tensors: %s",
            len(trainable_state),
            ckpt_path,
        )
        return ckpt_path
        ...
        while self.global_step < self.max_steps:
            ...
            with self.accelerator.accumulate(self.model):
                ...
                if self.accelerator.sync_gradients:
                    ...
                    self.recent_global_losses.append(global_loss)
                    loss_avg50 = float(sum(self.recent_global_losses) / len(self.recent_global_losses))
                    global_loss_metrics = {}
                    ...
                    if self.log_every > 0 and self.global_step % self.log_every == 0 and self.accelerator.is_main_process:
                        ...
                        description = "[train] epoch=%d step=%d/%d loss=%.4f loss_avg50=%.4f " % (
                            self.epoch,
                            self.global_step,
                            self.max_steps,
                            global_loss,
